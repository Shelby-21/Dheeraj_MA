# -*- coding: utf-8 -*-
"""loan_default.py - Final Code with Theme 6 (Vivid) and Tab Layout"""
...
def load_data():
    try:
        ...
        df = pd.read_excel('Loan_Dheeraj.xlsx') 
        return df
...
if df is not None:
    ...
    # --- MIDDLE SECTION: THE DRIVERS (NEW LAYOUT: TABS - Theme 6 Applied) ---
    st.subheader("The Drivers: Risk Analysis")
    
    # Create the tab containers
    tab_emp, tab_cred = st.tabs(["Risk by Employment Type", "Risk by Credit History"])

    # Chart 1: Risk by Employment (inside tab)
    with tab_emp:
        ...
        fig_emp = px.bar(emp_risk, x='Employment_Type', y='Default Rate (%)',
                         ...
                         color='Default Rate (%)', 
                         color_continuous_scale='Turbo') # <--- THEME 6: TURBO
        st.plotly_chart(fig_emp, use_container_width=True)

    # Chart 2: Risk by Credit History (inside tab)
    with tab_cred:
        cred_risk = get_risk_by_category('Credit_History')
        fig_cred = px.bar(cred_risk, x='Credit_History', y='Default Rate (%)',
                          title="Default Rate by Credit History",
                          color='Default Rate (%)', 
                          color_continuous_scale='Turbo') # <--- THEME 6: TURBO
